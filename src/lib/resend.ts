  ...
  await resend.emails.send({
    ...
    html: `
      <div style="background:#0C0516;color:#F0E6FF;font-family:Inter,sans-serif;padding:40px;max-width:600px;margin:0 auto;border-radius:16px;border:1px solid rgba(127,58,161,0.35)">
        ...
        <p style="color:#9B8FC0;margin-bottom:24px">Hi ${esc(name)}, your order has been confirmed.</p>
        <div style="background:rgba(84,22,181,0.08);border:1px solid rgba(127,58,161,0.35);border-radius:12px;padding:20px;margin-bottom:24px">
          ${items.map(i => `<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid rgba(127,58,161,0.2)"><span>${esc(i.title)}</span><span style="color:#F0830B">$${Number(i.price).toFixed(2)}</span></div>`).join('')}
          <div style="display:flex;justify-content:space-between;padding-top:12px;font-weight:bold"><span>Total</span><span style="color:#F0830B">$${Number(total).toFixed(2)}</span></div>
        </div>
        <p style="color:#9B8FC0;font-size:14px">Order ID: ${esc(orderId)}</p>
