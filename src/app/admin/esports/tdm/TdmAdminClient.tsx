...
export default function TdmAdminClient({ packages: init }: { packages: PkgItem[] }) {
  ...
  async function save() {
    ...
    const body = {
      ...
      price: 0, category: 'PACKAGE', subCategory: 'tdm',
  ...
  return (
    <div style={{ minHeight:'100vh', background:'linear-gradient(180deg,#0F083B,#0C0516)', padding:'2rem', fontFamily:"'Montserrat',sans-serif", direction:'rtl' }}>
      <style>{`
        .ta-btn { border:none; cursor:pointer; font-family:inherit; transition:all .2s; }
        .ta-input { width:100%; padding:.7rem 1rem; border-radius:10px; background:rgba(255,255,255,0.05); border:1px solid rgba(84,22,181,0.3); color:#e8e4f8; font-size:.9rem; font-family:inherit; outline:none; }
        .ta-input:focus { border-color:#7F3AA1; background:rgba(84,22,181,0.08); }
        .ta-input::placeholder { color:rgba(180,170,210,0.35); }
        .ta-upload-btn { display:inline-flex; align-items:center; gap:8px; padding:.6rem 1.2rem; border-radius:10px; border:1px dashed rgba(84,22,181,0.5); background:rgba(84,22,181,0.07); color:rgba(155,89,208,0.8); font-size:.82rem; font-weight:600; cursor:pointer; transition:all .2s; }
        .ta-upload-btn:hover { border-color:#7F3AA1; background:rgba(84,22,181,0.14); color:#c4a0e0; }
        .ta-section-label { font-size:.7rem; font-weight:700; letter-spacing:2px; text-transform:uppercase; color:rgba(155,89,208,0.6); margin-bottom:.6rem; display:block; }
        .ta-toggle { display:flex; align-items:center; gap:10px; cursor:pointer; user-select:none; }
        .ta-toggle-box { width:40px; height:22px; border-radius:12px; position:relative; transition:background .2s; }
        .ta-toggle-thumb { position:absolute; top:3px; width:16px; height:16px; border-radius:50%; background:#fff; transition:left .2s; }
        .ta-reorder-row { display:flex; align-items:center; gap:10px; padding:.65rem .9rem; border-radius:10px; background:rgba(255,255,255,0.03); border:1px solid rgba(84,22,181,0.15); margin-bottom:6px; cursor:grab; user-select:none; }
        .ta-reorder-row:active { cursor:grabbing; background:rgba(84,22,181,0.1); }
        .ta-reorder-thumb { width:44px; height:32px; border-radius:6px; object-fit:cover; flex-shrink:0; }
        .ta-reorder-url { flex:1; font-size:.7rem; color:rgba(170,155,200,0.55); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; direction:ltr; }
        .ta-type-badge { font-size:.58rem; font-weight:700; letter-spacing:1px; text-transform:uppercase; padding:2px 7px; border-radius:8px; flex-shrink:0; }
        .ta-card { background:rgba(15,8,59,0.6); border:1px solid rgba(84,22,181,0.2); border-radius:14px; overflow:hidden; transition:all .25s; }
        .ta-card:hover { border-color:rgba(127,58,161,0.5); transform:translateY(-3px); box-shadow:0 8px 24px rgba(84,22,181,0.2); }
        .ta-card-cover { width:100%; height:160px; object-fit:cover; display:block; background:rgba(84,22,181,0.1); }
        .ta-card-body { padding:14px 16px; }
        .ta-card-title { font-size:.95rem; font-weight:700; color:#e8e4f8; margin-bottom:8px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
        .ta-card-meta { font-size:.72rem; color:rgba(155,89,208,0.6); display:flex; gap:12px; margin-bottom:12px; }
        .ta-card-actions { display:flex; gap:8px; }
        .ta-media-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(72px,1fr)); gap:8px; }
        .ta-media-item { position:relative; border-radius:8px; overflow:hidden; aspect-ratio:1; background:rgba(84,22,181,0.08); }
        .ta-media-item img { width:100%; height:100%; object-fit:cover; display:block; }
        .ta-media-item-video { width:100%; height:100%; display:flex; align-items:center; justify-content:center; background:rgba(84,22,181,0.12); }
        .ta-media-del { position:absolute; top:4px; right:4px; width:20px; height:20px; border-radius:50%; background:rgba(0,0,0,0.75); border:none; color:#fff; cursor:pointer; display:flex; align-items:center; justify-content:center; font-size:.6rem; opacity:0; transition:opacity .2s; }
        .ta-media-item:hover .ta-media-del { opacity:1; }
        .ta-media-type { position:absolute; bottom:3px; left:3px; font-size:.52rem; font-weight:700; padding:1px 5px; border-radius:4px; }
      `}</style>

      {/* Page Header */}
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'2rem', flexWrap:'wrap', gap:'1rem' }}>
        <div>
          <div style={{ fontSize:'.65rem', fontWeight:700, letterSpacing:'3px', textTransform:'uppercase', color:'rgba(155,89,208,0.6)', marginBottom:6 }}>TDM ESPORTS</div>
          <h1 style={{ fontSize:'clamp(1.4rem,3vw,2rem)', fontWeight:900, color:'#eae6ff', margin:0 }}>إدارة الباكدجات</h1>
        </div>
        <button className="ta-btn" onClick={openAdd} style={{ display:'flex', alignItems:'center', gap:8, padding:'.75rem 1.6rem', borderRadius:50, background:'linear-gradient(135deg,#5416B5,#7F3AA1)', color:'#fff', fontWeight:700, fontSize:'.9rem', boxShadow:'0 4px 16px rgba(84,22,181,0.4)' }}>
      ...
      {packages.length === 0 ? (
        <div style={{ textAlign:'center', padding:'60px 20px', color:'rgba(155,89,208,0.3)' }}>
          ...
          <p style={{ fontSize:'.9rem' }}>لا توجد باكدجات — ابدأ بإضافة أول باكدج TDM</p>
        </div>
      ) : (
          ...
          {packages.map(p => {
            ...
            return (
              <div key={p.id} className="ta-card">
                {p.imageUrl
                  ? /* eslint-disable-next-line @next/next/no-img-element */ <img src={p.imageUrl} alt={p.title} className="ta-card-cover"/>
                  : <div className="ta-card-cover" style={{ display:'flex', alignItems:'center', justifyContent:'center', color:'rgba(84,22,181,0.3)' }}><ImagePlus size={32}/></div>
                }
                <div className="ta-card-body">
                  <div className="ta-card-title">{p.title}</div>
                  ...
                  <div className="ta-card-actions">
                    <button className="ta-btn" onClick={() => openEdit(p)} style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', gap:6, padding:'.5rem', borderRadius:8, background:'rgba(84,22,181,0.15)', border:'1px solid rgba(84,22,181,0.3)', color:'#c4a0e0', fontSize:'.8rem', fontWeight:600 }}>
                      <Edit2 size={13}/> تعديل
                    </button>
                    <button className="ta-btn" onClick={() => del(p.id)} disabled={deleting === p.id} style={{ padding:'.5rem .8rem', borderRadius:8, background:'rgba(220,50,50,0.1)', border:'1px solid rgba(220,50,50,0.25)', color:'rgba(220,100,100,0.8)' }}>
          ...
          >
            ...
            >
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'1.2rem 1.5rem', borderBottom:'1px solid rgba(84,22,181,0.2)', background:'rgba(84,22,181,0.06)' }}>
                <div>
                  <div style={{ fontSize:'.62rem', fontWeight:700, letterSpacing:'3px', textTransform:'uppercase', color:'rgba(155,89,208,0.6)', marginBottom:4 }}>TDM ESPORTS</div>
                  <div style={{ fontWeight:800, color:'#eae6ff', fontSize:'1rem' }}>{editId ? 'تعديل الباكدج' : 'إضافة باكدج جديد'}</div>
                </div>
                <button className="ta-btn" onClick={closeModal} style={{ width:34, height:34, borderRadius:'50%', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.12)', color:'rgba(200,185,230,0.7)', display:'flex', alignItems:'center', justifyContent:'center' }}>
                  <X size={15}/>
                </button>
              </div>
              ...
              <div style={{ padding:'1.5rem' }}>
                ...
                <div style={{ marginBottom:'1.2rem' }}>
                  <span className="ta-section-label">اسم الباكدج</span>
                  <input className="ta-input" placeholder="مثال: باكدج فريق TDM" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))}/>
                </div>

                {/* Cover */}
                <div style={{ marginBottom:'1.4rem' }}>
                  <span className="ta-section-label">صورة الغلاف</span>
                  <div style={{ display:'flex', alignItems:'center', gap:12, flexWrap:'wrap' }}>
                    <button className="ta-upload-btn ta-btn" onClick={() => coverRef.current?.click()} disabled={uploading==='cover'}>
                    ...
                    {form.imageUrl && (
                      <div style={{ position:'relative', width:64, height:48, borderRadius:8, overflow:'hidden', border:'1px solid rgba(84,22,181,0.3)', flexShrink:0 }}>
                        ...
                        <button className="ta-btn" onClick={()=>setForm(f=>({...f,imageUrl:''}))} style={{ position:'absolute', top:2, right:2, width:16, height:16, borderRadius:'50%', background:'rgba(0,0,0,0.8)', color:'#fff', fontSize:'.55rem', display:'flex', alignItems:'center', justifyContent:'center', border:'none' }}>✕</button>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div style={{ display:'flex', gap:10, marginBottom:'.6rem', flexWrap:'wrap' }}>
                  ...
                  <button className="ta-upload-btn ta-btn" onClick={() => videoRef.current?.click()} disabled={uploading==='video'}>
                    <Video size={14}/>{uploading==='video' ? 'جاري رفع الفيديو...' : `رفع فيديو (${vidCount})`}
                  </button>
                  {uploadErr && <p style={{ width:'100%', margin:0, fontSize:'.72rem', color:'#f87171' }}>{uploadErr}</p>}
                </div>
                ...
                {form.media.length > 0 && (
                  <div style={{ marginBottom:'1.4rem' }}>
                    <span className="ta-section-label">المحتوى المضاف ({form.media.length})</span>
                    <div className="ta-media-grid">
                      {form.media.map(item => (
                        <div key={item.id} className="ta-media-item">
                          ...
                          <button className="ta-media-del ta-btn" onClick={() => removeMedia(item.id)}><X size={10}/></button>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {form.media.length > 0 && (
                  <div style={{ marginBottom:'1.4rem', border:'1px solid rgba(84,22,181,0.25)', borderRadius:14, overflow:'hidden' }}>
                    <button className="ta-btn" onClick={() => setShowReorder(v => !v)}
                                ...
                                <Reorder.Item key={item.id} value={item}>
                                  <div className="ta-reorder-row">
                                    ...
                                    {item.type === 'image'
                                      ...
                                      : videoPoster(item.url)
                                        ? /* eslint-disable-next-line @next/next/no-img-element */ <img src={videoPoster(item.url)!} alt="" className="ta-reorder-thumb"/>
                                    ...
                                    <span className="ta-reorder-url">{item.url.split('/').pop()}</span>
                                    <button className="ta-btn" onClick={() => removeMedia(item.id)} style={{ width:22, height:22, borderRadius:'50%', background:'rgba(220,50,50,0.1)', border:'1px solid rgba(220,50,50,0.2)', color:'rgba(220,100,100,0.7)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                                      <X size={10}/>
                                    </button>
                                  </div>
                                </Reorder.Item>
                ...
                <div style={{ display:'flex', gap:'1.5rem', marginBottom:'1.6rem', flexWrap:'wrap' }}>
                  {[
                    ...
                    <label key={t.key} className="ta-toggle">
                      <div className="ta-toggle-box" style={{ background: form[t.key as keyof FormState] ? t.color : 'rgba(255,255,255,0.1)' }} onClick={() => setForm(f => ({ ...f, [t.key]: !f[t.key as keyof FormState] }))}>
                        <div className="ta-toggle-thumb" style={{ left: form[t.key as keyof FormState] ? 21 : 3 }}/>
                      </div>
                ...
                <div style={{ display:'flex', gap:10 }}>
                  <button className="ta-btn" onClick={save} disabled={saving || !form.title.trim()}
                  ...
                  <button className="ta-btn" onClick={closeModal}
